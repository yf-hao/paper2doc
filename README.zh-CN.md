...
Pandoc 是可选工具。安装后，检测到的显示公式会优先走
`LaTeX → Word OMML` 路径；未安装或转换失败时，程序会自动使用 PDF 原始区域
图片兜底：
...
- 双栏左栏末尾、右栏开头以及分页处的文字，如果内容是同一段的延续，会合并为
  一个逻辑段落；标题、列表、图注、表格和真正的新段落不会被强制合并。
- 显示公式会从正文文字中单独识别，优先转换为 LaTeX 和 Word OMML；如果 Pandoc
  不可用或转换失败，则根据 PDF 原始区域渲染为图片写入 DOCX。公式符号、上下标、
  求和范围和公式编号不会发送给翻译服务。
- 正文中的 `Fig. 6` 不会强行移动图片。
...
- 文字型 PDF 表格优先使用 PyMuPDF 的原生表格接口识别；对于有横线、无竖线
  的常见表格，程序会结合横线和文本空白推断行列，并从普通段落中排除表格文字，
  避免重复提取。
- 成功识别的表格写成 Word 原生表格，英文表格下方紧跟对应的中文原生表格。
  保留标题、空单元格、近似列宽、表头、合并单元格以及可推断的横线。
- 如果只能确认表格区域但无法可靠恢复单元格网格，则把原始表格区域作为图片
  插入，避免静默丢失内容。
