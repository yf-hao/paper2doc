...
数学公式不能全部按照普通文字块写入 Word。PDF 中的公式通常由多个相互重叠的字形、
上下标和隐藏文字层组成，直接按坐标拼接会造成符号重复、顺序错误以及上下标丢失。
因此，需要区分块级公式和行内公式：

1. 块级公式根据公式符号、短文字块和空间邻近关系识别公式区域。
2. 块级公式将公式编号并入同一个区域，按 PDF 原始区域渲染为图片。
3. 行内公式根据 span 的字号、基线和坐标识别上下标。
4. 行内公式写入 Word 原生数学对象，避免将 `uₓ` 展平为 `ux`。
5. 公式内容从普通正文提取结果中排除，避免重复文字进入段落或翻译批次。

块级公式使用 PDF 图片可以最大限度保留复杂公式的视觉结构；行内公式使用 Word
数学对象可以保留与正文同一行的上下标结构。
