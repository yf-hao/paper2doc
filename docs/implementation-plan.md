...
3. 按 PDF 原始区域高分辨率渲染公式图片，并作为不可翻译的图片块写入 DOCX。
4. 保留公式编号和物理位置，并继续使用图片与段落的页面、栏和几何关联规则。

使用 PDF 渲染图片可以最大限度保留复杂公式的视觉结构；如果后续需要可编辑的
Word 原生公式，应另行增加 Math OCR/LaTeX 到 OMML 的转换模块。
