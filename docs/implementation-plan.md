            ...
            messages=[
                {
                    "role": "system",
                    "content": (
                        "你是学术论文翻译助手。将英文完整、准确地翻译为中文，"
                        "不得删减、概括、跳过或改写任何单词、短语、句子、段首或段尾内容。"
                        "保留公式、变量名、引用编号和专业术语。翻译前后请在内部核对原文首尾，"
                        "确保译文覆盖完整。只返回中文译文，不添加解释。"
                    ),
                },
...
- 不得遗漏段落开头、结尾、从句、引用前后的任何内容；非空但不完整的译文不能视为成功。
- 专业术语前后一致。
- 公式、变量名和引用编号不被错误翻译。
...
段落合并必须在栏识别之后进行。普通的同栏合并依据垂直间距和起始位置；
如果左栏末尾和右栏开头（或上一页末尾和下一页开头）在阅读顺序上相邻，
且后者是前者的句子延续，则也要合并，不能把栏边界或分页误认为段落边界。
如果前一段已经以句号等结束标点结束，或后一段是标题、列表、图注、表格或图片
之后的独立内容，则保持为新的段落。跨栏或跨页合并时保留每个物理文字块的位置，
以便后续图片仍能关联到正确的页面和栏。
...
数学公式不能按照普通文字块写入 Word。PDF 中的公式通常由多个相互重叠的字形、
上下标和隐藏文字层组成，直接按坐标拼接会造成符号重复、顺序错误以及上下标丢失。
因此，显示公式需要单独处理：

1. 根据公式符号、短文字块和空间邻近关系识别公式区域。
2. 将公式区域从普通正文提取结果中排除，避免重复文字进入段落或翻译批次。
3. 将识别出的公式文字转换为 LaTeX，并优先通过 Pandoc 转换为 Word OMML；
   如果 LaTeX 或 OMML 转换失败，再按 PDF 原始区域高分辨率渲染公式图片。
4. 保留公式编号和物理位置，并继续使用公式块与段落的页面、栏和几何关联规则。

Pandoc 不可用时仍会自动走 PDF 图片兜底，因此公式不会因为缺少外部转换工具而丢失。
